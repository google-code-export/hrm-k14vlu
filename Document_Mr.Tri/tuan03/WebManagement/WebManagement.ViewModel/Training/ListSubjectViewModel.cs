...
{
    [Export(ViewModelTypes.ListSubjectViewModel, typeof(ViewModelBase))]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class ListSubjectViewModel : ViewModelBase, IViewModel
    {
        #region Private Members

        private ITrainingModel _modelTrain;
        public const string ViewKey = "WM_LISTSUBJECT";
        ...
        public ListSubjectViewModel(ITrainingModel modelTrain)
        {
        ...
        private void OnAddClickCommand()
        {
            ...
            {
                SelectedItem = null;
                CallDialog.Show(this, ViewTypes.EditSubjectView, c =>
                {
                ...
                if (SelectedItem != null)
                {
                    if (!_modelTrain.IsBusy)
                    {
                        MessageCustomize.Show("Bạn muốn xóa môn học này?", "Xóa", MessageBoxButton.OKCancel, MessageImage.Question, c =>
                        {
                            ...
                            {
                                IsLoading = true;
                                _modelTrain.DeleteSubjectAsync(SelectedItem);
                            }
                        });
                    }
                }
        ...
        public void LoadInitComplete()
        {
            if (_listData != null)
            {
                ...
                lst.Add(new CustomizeSubjectType { TypeID = 0, TypeName = "Chuyên nghành" });
                lst.Add(new CustomizeSubjectType { TypeID = 1, TypeName = "Đại cương" });
                ListType = lst;
            }
        }
        ...
        private void _modelTrain_DeleteSubjectComplete(object sender, SubmitOperationEventArgs e)
        {
            try
            {
                IsLoading = false;

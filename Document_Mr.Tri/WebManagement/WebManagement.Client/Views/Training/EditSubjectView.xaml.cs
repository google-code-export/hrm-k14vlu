...
namespace WebManagement.Client
{
    public partial class EditSubjectView : Telerik.Windows.Controls.RadWindow
    {
        ...
        public EditSubjectView(DialogMessage dialog)
        {
            InitializeComponent();
            try
            {
                if (!ViewModelBase.IsInDesignModeStatic)
                {
                    _viewModelExport = App.Container.GetExport<ViewModelBase>(
                        ViewModelTypes.EditSubjectViewModel);
        ...
        private void View_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                ...
                {
                    var binding = txtMaMonHoc.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();
                    binding = txtTenMonHoc.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();

...
namespace WebManagement.Client
{
    public partial class EditDepartmentView : Telerik.Windows.Controls.RadWindow
    {
        ...
        public EditDepartmentView(DialogMessage dialog)
        {
            InitializeComponent();
            try
            {
                if (!ViewModelBase.IsInDesignModeStatic)
                {
                    _viewModelExport = App.Container.GetExport<ViewModelBase>(
                        ViewModelTypes.EditDepartmentViewModel);
        ...
        private void View_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                ...
                {
                    var binding = txtMaKhoa.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();
                    binding = txtTenKhoa.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();

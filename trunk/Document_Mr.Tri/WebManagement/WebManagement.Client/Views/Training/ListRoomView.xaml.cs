...
{
    public partial class ListRoomView : UserControl
    {
            ...
            try
            {
                if (!ViewModelBase.IsInDesignModeStatic)
                {
                    _viewModelExport = App.Container.GetExport<ViewModelBase>(
                        ViewModelTypes.ListRoomViewModel);
        ...
        private void View_SetFocus(object sender, EventArgs e)
        {
            try
            {
                if (sender is List<CustomizeBranch>)
                {
                    ...
                    if (col != null)
                    {
                        col.ItemsSource = sender as List<CustomizeBranch>;
                    }
                }
            }
            catch (Exception ex)

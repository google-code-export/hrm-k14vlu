...
namespace WebManagement.Client
{
    public partial class EditSubjectSummaryView : Telerik.Windows.Controls.RadWindow
    {
        ...
        public EditSubjectSummaryView(DialogMessage dialog)
        {
            InitializeComponent();
            try
            {
                if (!ViewModelBase.IsInDesignModeStatic)
                {
                    _viewModelExport = App.Container.GetExport<ViewModelBase>(
                        ViewModelTypes.EditSubjectSummaryViewModel);
        ...
        private void View_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                ...
                {
                    var binding = txtMalop.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();
                    binding = txtTenLop.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();

...
{
    public partial class EditRoomView : Telerik.Windows.Controls.RadWindow
    {
        private Lazy<ViewModelBase> _viewModelExport;

        public EditRoomView(DialogMessage dialog)
        {
            InitializeComponent();
            try
            {
                if (!ViewModelBase.IsInDesignModeStatic)
                {
                    _viewModelExport = App.Container.GetExport<ViewModelBase>(
                        ViewModelTypes.EditRoomViewModel);
        ...
        private void View_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                ...
                {
                    var binding = txtMaPhong.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();
                    binding = txtTenPhong.GetBindingExpression(TextBox.TextProperty);
                    binding.UpdateSource();
